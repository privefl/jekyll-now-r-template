FormatPost <- function(rmd, knitr_files_dir = "knitr_files", 
                       tmp_dir = "_built") {
  # rmd: name/path to specific Rmd file to convert
  # knitr_files_dir: The permanent directory where the files like plots
  #                  are put
  # tmp_dir: temporary directory for the temporary html file
  ...
  new.header <- paste("---", lines.rmd[title], lines.rmd[date],
                      "layout: post", "---", "", sep = "\n")
  ...
  for (i in ind) {
    lines.html[i] <-
      gsubfn::gsubfn(pattern, file.path("{{ site.baseurl }}", knitr_files_dir,
                                        tmp[[i]][1]), lines.html[i])
  }
  dir.source <- file.path(tmp_dir, pattern)
  if (!dir.exists(knitr_files_dir)) dir.create(knitr_files_dir)
  file.copy(from = dir.source, to = knitr_files_dir, 
